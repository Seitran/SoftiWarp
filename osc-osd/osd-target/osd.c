...
static inline void osd_invalidate_id_cache(struct osd_device *osd)
{
	pthread_mutex_lock(&osd->ic->lock);
	osd->ic->cur_pid = osd->ic->next_id = 0;
	pthread_mutex_unlock(&osd->ic->lock);
}

static int osd_get_nextoid(struct osd_device *osd, uint64_t pid, uint64_t *oid,
			   int numoid)
{
	int ret = 0;
	pthread_mutex_lock(&osd->ic->lock);
	...
	if (osd->ic->cur_pid == pid) { /* cache hit */
		*oid = osd->ic->next_id;
		osd->ic->next_id += numoid;
	} else {
		int ret = obj_get_nextoid(osd->dbc, pid, oid);
		if (ret != 0)
			goto out_err;
		osd->ic->cur_pid = pid;
		osd->ic->next_id = *oid + numoid;
	}
	if (*oid == 1) {
...
static int osd_get_nextcid(struct osd_device *osd, uint64_t pid, uint64_t *cid)
{
	int ret = 0;
	...
	if (*cid == 1) {
		*cid = COLLECTION_OID_LB; /* first oid in partition */
		osd->ic->next_id = *cid + 1;
	}  
...
static pthread_mutex_t osd_open_lock = PTHREAD_MUTEX_INITIALIZER;

/* _osd_open initializes the parts of the osd_device that are common
   to all threads - the root path, the id cache, and also the
   directory structure (if necessary).  The db_context is per-thread
   so that we have separate connections per thread.  While we could
   have one connection shared amongst therads, this doesn't allow
   transactions, because transactions could get mixed on the same
   connection.  _osd_open is called by a single thread with the help
   of the osd_open_lock.
*/

static int _osd_open(const char *root, struct osd_device *osd, int connect_to_db)
{
	int i = 0;
	int ret = 0;
	char path[MAXNAMELEN];
...
init_db:
	...
	osd_invalidate_id_cache(osd);
out:
	if (ret != 0)
		osd_error("!_osd_open => %d", ret);

	return ret;
}

/* this routine initializes thread-specific data */
int osd_open(const char *root, struct osd_device *osd)
{
	int ret = 0;
	static char * _root = NULL;
	static struct id_cache _ic;

	memset(osd, 0, sizeof(*osd));
	osd->ic = &_ic;

	/* use osd_open_lock to make sure only one thread calls
	   _osd_open.  We could use pthread_once() but that doesn't
	   allow init_routines with arguments.
	*/
	pthread_mutex_lock(&osd_open_lock);
	if (!_root) {
		ret = _osd_open(root, osd, 1);
		_root = osd->root;
		pthread_mutex_init(&osd->ic->lock, NULL);
	} else {
		char path[MAXNAMELEN];
	...
	pthread_mutex_unlock(&osd_open_lock);
			  ...
			  uint8_t dupl_method, uint8_t *sense)
{
	...
	if (requested_oid == 0) {
		ret = osd_get_nextoid(osd, pid, &oid, 1);
		if (ret != 0)
			goto out_hw_err;
	} else {
	       ...
	       uint16_t numoid, uint32_t cdb_cont_len, uint8_t *sense)
{
	...
	if (requested_oid == 0) {
		ret = osd_get_nextoid(osd, pid, &oid, numoid);
		if (ret != 0)
			goto out_hw_err;
	} else {
		ret = obj_ispresent(osd->dbc, pid, requested_oid, &present);
			  ...
			  uint64_t requested_cid, uint32_t cdb_cont_len, uint8_t *sense)
{
	...
	if (requested_cid == 0) {
		 ...
		 * oid using SQL itself
		 */
		ret = osd_get_nextcid(osd, pid, &cid);
		if (ret != 0)
			goto out_hw_err;
	} else {
...
int osd_format_osd(struct osd_device *osd, uint64_t capacity, uint32_t cdb_cont_len, uint8_t *sense)
{
	...
	/* do not close the OSD or empty the md directory - just
	   delete the data from the db.  if we close the OSD, then the
	   dbc pointers in other threads will no longer be valid
...
create:
	ret = _osd_open(root, osd, 0); /* will create files/dirs under root */
	if (ret != 0) {
...
int osd_get_member_attributes(struct osd_device *osd, uint64_t pid,
			      uint64_t cid, uint64_t min_oid, uint64_t max_oid, uint64_t oids[], int numobj,
			      uint32_t cdb_cont_len, uint8_t *sense)
{
	int ret = 0;
	size_t i = 0;
	...
	osd_debug("%s: get attrs from pid %llu cid %llu", __func__, llu(pid),
		  llu(cid));
	...
	ret =   coll_get_oids_in_cid(osd->dbc, pid, cid, min_oid, max_oid,
					    numobj*8, (uint8_t *)oids,
					    &usedlen, &addlen, &cont_id);
       
        for(j=0; j<numobj; j++) {
		oids[j] = get_ntohll(&oids[j]);
	}
	if (ret != 0)
		goto out_hw_err;
	...
	if (list_attr == 0)  {
		...
		ret = (cid == 0 ?
		       obj_get_cids_in_pid(osd->dbc, pid, initial_oid,
					   ...
					   used_outlen, &add_len, &cont_id)
		       :
		       coll_get_oids_in_cid(osd->dbc, pid, cid, initial_oid, 1,
					    alloc_len, &outdata[24],
					    used_outlen, &add_len, &cont_id));
		if (ret)
			goto out_hw_err;
			  ...
			  uint8_t fcr, uint32_t cdb_cont_len, uint8_t *sense)
{
	...
	if (ret != 0)
        ...
        /* delete all collection memberships */
        ret = coll_delete_oid(osd->dbc, pid, cid);
        if (ret != 0)
                goto out_hw_err;
	    ...
	    uint8_t *sense)
{
	...
	obj_type = get_obj_type(osd, pid, oid);
	if (obj_type != USEROBJECT && obj_type != COLLECTION) 
		goto out_cdb_err;
