...
int coll_initialize(struct db_context *dbc)
{
	...
	sprintf(SQL, "SELECT oid FROM %s WHERE cid = ? AND oid BETWEEN ? AND ?;", dbc->coll->name);
...
int coll_get_oids_in_cid(struct db_context *dbc, uint64_t pid, uint64_t cid, 
		       uint64_t min_oid, uint64_t max_oid, uint64_t alloc_len, 
...
repeat:
	...
	ret |= sqlite3_bind_int64(stmt, 1, cid);
	ret |= sqlite3_bind_int64(stmt, 2, min_oid);
	ret |= sqlite3_bind_int64(stmt, 3, max_oid);
