 ...
 */
static int get_attr_list(struct command *cmd, uint64_t pid, uint64_t *oids,
			 uint8_t isembedded, uint16_t numoid, uint32_t cdb_cont_len)
{
	int ret = 0;
	int err = 0;
	...
	list_alloc_len -= 8;
	len_per_oid = list_len / numoid;
	starting_len = list_len;
	while (list_len > 0) {
		uint32_t page = get_ntohl(&list_hdr[0]);
		uint32_t number = get_ntohl(&list_hdr[4]);
		uint32_t get_used_outlen = 0;

		ret = osd_getattr_list(cmd->osd, pid, oids[current_oid], page, number,
		...
		list_hdr += 8;
                ...
                if (!((starting_len - list_len) % len_per_oid))
                    current_oid++;
	}
			  ...
			  uint16_t numoid, uint32_t cdb_cont_len)
{
	...
	if (cmd->getset_cdbfmt == GETPAGE_SETVALUE) {
	        ...
	        if(numoid > 1) {
			uint64_t *oids = (uint64_t *)malloc(numoid * sizeof(uint64_t));
	       		ret = get_attr_list(cmd, pid, oids, isembedded, numoid, cdb_cont_len);
			free(oids);
			return ret;
		} else {
			return get_attr_list(cmd, pid, &oid, isembedded, 1, cdb_cont_len);
		}
...
//	osd_debug("%s: start 0x%04x", __func__, cmd->action);
	...
	case OSD_GET_MEMBER_ATTRIBUTES: {
		uint64_t pid = get_ntohll(&cdb[16]);
		uint64_t cid = get_ntohll(&cdb[24]);
		uint64_t min_oid = get_ntohll(&cdb[32]);
		uint64_t max_oid = get_ntohll(&cdb[40]);
		int numobj = get_ntohl(&cmd->cdb[80]);
		uint64_t oids[numobj];

		ret = check_no_continuations(cmd, cdb_cont_len, pid, cid);
		if (ret)
			break;
              
                ret = osd_get_member_attributes(osd, pid, cid, min_oid, max_oid, oids, numobj, cdb_cont_len, sense);
		if (ret)
			break;

		ret = get_attr_list(cmd, pid, oids, true, numobj, cdb_cont_len);
		break;
	}
	case OSD_LIST: {
		ret = cdb_list(cmd);
