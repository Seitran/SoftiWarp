...
static int submit_command(int fd, struct osd_command *command,
			  uint8_t *buf __attribute__((unused)))
{
	int ret;

	/* async does not work when you have multiple processes, since
	   one process's command response may get read by another process
	*/
	int use_async = 0;
...
int inquiry(int fd)
{
	...
	submit_command(fd, &command, inquiry_rsp);
	 ...
	 uint64_t alloc_len, int list_attr)
{
	...
	*/
	...
	submit_command(fd, &command, &command);
